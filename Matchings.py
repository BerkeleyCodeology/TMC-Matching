...
full_students = pd.read_csv("students.csv")# minor preprocessing of original dataframes for better organization later
...
full_students = full_students.rename(columns={'Student Name - Last, First': 'Student'})# edit values in Private/Class Tutoring columns to be just number of students (0 - 4)
...
# full_tutors and full_students dataframes will not be mutated after this# split students into the 4 priorities 
...
students5 = full_students.loc[(full_students[application] == newStudent) & (full_students[freeLunch] == 'No')]#split classes into separate rows
...
full_classes# more useful arrays
student = ['Student']
...
classes = extract_times(full_classes)# copies to mutate in function; tutors will be removed once they are fully matched
...
def match_classes(classes):
    ...
    # no match found for any of the three instrument choices
    return [None, None, None, None]# create the iterator from rows of students
s_without_count = students.drop(columns='Count')
students_iter = s_without_count.iterrows()# create the dataframe to store matches once made
matches = pd.DataFrame(columns=['Student', 'Tutor', 'Instrument', 'Time(s)'])
class_matches = pd.DataFrame(columns=['Class', 'Tutor', 'Instrument', 'Time(s)', 'id'])# iterate through students; use function defined above to get a match
...
class_matchestimesToTutors = {' 8AM': ' Before 11am', ' 9AM': ' Before 11am', '10AM':'Before 11am', '11AM':'11am-1pm', '12PM':'11am-1pm', ' 1PM':' 1pm-3pm', ' 2PM':' 1pm-3pm', ' 3PM':' 3pm-5pm', ' 4PM':' 5pm'}
...
class_match_2# iterate through students; use function defined above to get a match
for s in students_iter:
    ...
    if m[0] is not None:
        ...
        if tutors2.at[tutor_idx, 'Private Tutoring'] == 0:
            ...
            tutors2.at[tutor_idx,m[2][0]] = False# matched students
matched = matches[matches['Tutor'].notna()]
matched# unmatched students
no_match = matches[matches['Tutor'].isna()]
no_match# unmatched tutors
tutors2[['Tutor', 'Instrument', 'Count', 'Private Tutoring']]# matches with relevant information (add any columns needed)
...
student_matched = matched.to_csv()class_matched = class_matches[class_matches['Tutor'].notna()]
...
class_matchedfull_classes# students with no matches with relevant information (add any columns needed)
...
no_match# time for computation (in seconds):
time.time() - start